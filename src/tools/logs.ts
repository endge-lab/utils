...
export class StructuredLogger extends Subscribable {
  ...
  start(context: string): this {
    this._currentContext.push(context)
    return this
  }
   ...
   */
    ...
    if (message) {
      ...
      const combinedActions = actions?.length
        ? actions
        : this._currentActions.length > 0 ? this._currentActions : undefined
      this._log(level ?? 'info', message, combinedActions)
    }

    this._currentContext.pop()
    this._currentActions = [] // сбрасываем экшены
    return this
  }
  ...
  action(icon: string, tooltip?: string, handler?: () => void): this {
    this._currentActions.push({ icon, tooltip, handler })
    return this
  }
  ...
  debug(message: string, actions?: StructuredLogEntry['actions']): void {
    this._log('debug', message, actions)
  }
  ...
  info(message: string, actions?: StructuredLogEntry['actions']): void {
    this._log('info', message, actions)
  }
  ...
  warn(message: string, actions?: StructuredLogEntry['actions']): void {
    this._log('warn', message, actions)
  }
  ...
  error(message: string, actions?: StructuredLogEntry['actions']): void {
    this._log('error', message, actions)
  }
  ...
  success(message: string, actions?: StructuredLogEntry['actions']): void {
    this._log('success', message, actions)
  }
