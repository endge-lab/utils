...
> {
   ...
   */
  constructor(predefinedEvents: Array<keyof StaticEvents> = [], private readonly _options: EventBusOptions = {}) {
  ...
  private _emit(event: string, payload?: any): void {
    ...
    // Состав текущей рассылки фиксируется до вызова пользовательского кода.
    for (const cb of [...callbacks]) {
      ...
      try {
        const result: unknown = cb(payload)
        if (result && typeof (result as PromiseLike<unknown>).then === 'function') {
          void Promise.resolve(result).catch(error => this._reportListenerError(error, event))
        }
      }
      catch (error) {
        this._reportListenerError(error, event)
      }
    }
  }

  private _reportListenerError(error: unknown, event: string): void {
    try {
      this._options.onListenerError?.(error, event)
    }
    catch {
      // Сбой обработчика ошибок также не должен прерывать независимых подписчиков.
    }
  }
  ...
  eventNames(): Array<string> {
    return [...this._listeners.entries()].filter(([, set]) => set.size > 0).map(([event]) => event)
  }
